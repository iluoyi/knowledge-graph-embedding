...
public class TransLMNN {
	...
	public static final float MU = 0.2f; 
...
//			}
			...
			for(int j = 0; j < trainExamples.size(); j++)
			{
				...
				float[] gradientEmb = embCalculator(posGradientEmb, "*", STEP_SIZE * MU);
				
				float[] tmpEmb = entityEmbeddings.get(posHeadEntity);
				entityEmbeddings.put(posHeadEntity, embCalculator(tmpEmb, "-", gradientEmb));
				tmpEmb = entityEmbeddings.get(posTailEntity);
				entityEmbeddings.put(posTailEntity, embCalculator(tmpEmb, "+", gradientEmb));
				tmpEmb = relationEmbeddings.get(relation);
				relationEmbeddings.put(relation, embCalculator(tmpEmb, "-", gradientEmb));	
				
				/*Large Margin Updating*/
				if(MARGIN + posDistance - negDistance > 0.0f)
				{
					...
					tmpEmb = entityEmbeddings.get(posHeadEntity);
	...
	{
		TransLMNN transLMNN = new TransLMNN();
